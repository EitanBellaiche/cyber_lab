...
BankBase    = declarative_base()   # בסיס חדש לבנק

class Person(PersonBase):
    __tablename__ = "person"
    username = Column(String(128), primary_key=True)
    # השדות האלה כבר לא בשימוש לאימות, אבל משאירים לסכימה קיימת
...
class Cred(CredBase):
    __tablename__ = "cred"
    username = Column(String(128), primary_key=True)
    password = Column(String(128))   # כאן נשמר ה-hash
    token    = Column(String(128))
